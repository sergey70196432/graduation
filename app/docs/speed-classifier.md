...
`model_*.tflite` и `labels.txt` должны браться из одного и того же экспорта. Порядок классов задаётся именно `labels.txt`, и при рассинхроне модель начнёт возвращать корректные индексы, но неправильные строки классов.
...
- **Сдвиг домена train → production**: качество сильно зависит от того, насколько train-данные похожи на реальные ROI. Поэтому в обучающем пайплайне используются усиленные ROI-подобные аугментации, а честную метрику лучше считать только на отложенном real ROI test-наборе.
