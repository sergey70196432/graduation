...
SELECT_CLASS_IDS = [0]  # пример: [0, 4, 10]

# Подмешивание внешнего датасета в формате YOLO (реальные кадры с разметкой)
...
# Базовое качество знака (0..1): 1.0 = идеально, 0.9 = даже крупные знаки чуть "портим"
...
# Пикселизация (мелкий знак => сильнее). Делается внутри ROI знака.
SIGN_PIXELATE_ENABLED = True
SIGN_PIXELATE_PROB = 0.90
SIGN_PIXELATE_SCALE_SMALL = (0.18, 0.45)  # во сколько раз уменьшать ROI перед апскейлом (0.2 = сильная пикселизация)
SIGN_PIXELATE_SCALE_LARGE = (0.75, 0.95)  # для крупных знаков (всё равно < 1.0, чтобы не было идеала)
...
def normalize_class_code(s):
    ...
    s = s.strip("'\"")
    # удаляем хвостовые точки
    while s.endswith("."):
        s = s[:-1]
        s = s.rstrip()
    # заменяем "._" на "_"
    s = s.replace("._", "_")
    # иногда встречаются двойные пробелы/подчёркивания
    s = re.sub(r"\s+", "", s)
    return s
...
def read_external_data_yaml(yaml_path):
    ...
    - train (путь/папка)
    - val (путь/папка)
    - names (список имён классов, как в вашем примере)
    """
    ...
    for line in lines:
        ...
        if line.startswith("path:"):
            ...
            # чаще всего names в одну строку: names: [...]
            ...
            if rhs.startswith("[") and rhs.endswith("]"):
                ...
                # если names дальше списком по строкам — соберём грубо
                data["names"] = []

    # подхватить многострочный вариант names:
...
def load_external_index(external_dir, our_code_to_id):
    """
    Индексация внешнего датасета:
    - external_names: список имён из data.yaml (индекс в labels = позиция в списке)
    - image -> labels (уже в наших class_id)
    - class_id -> set(images) (где этот класс встречается)
    - instances_per_class
    """
    index = {
        "images": {},  # img_path -> [(class_id, xc, yc, w, h), ...]
        "images_by_class": {},  # class_id -> set(img_path)
        "instances_per_class": {},  # class_id -> int
        "enabled": False,
    }
    ...
    # Собираем все label-файлы (train/val). Если train/val не указаны — ищем рекурсивно.
    label_files = []
    train_dir = meta.get("train")
    val_dir = meta.get("val")
    if train_dir:
        # обычно это 'train/images' или 'train' — нам нужны labels
        label_files += glob(os.path.join(external_dir, "train", "labels", "*.txt"))
    if val_dir:
        label_files += glob(os.path.join(external_dir, "valid", "labels", "*.txt"))
        label_files += glob(os.path.join(external_dir, "val", "labels", "*.txt"))
    ...
    def map_external_code_to_our_id(code):
        ...
        # 2) совпадение по "токенам" (например 5.19.1 попадёт в 5.19.1-5.19.2)
        token_hits = []
        ...
        # 3) fallback: вхождение подстрокой (если ровно 1 кандидат)
        sub_hits = []
        for k, cid in our_code_to_id.items():
    ...
    for lp in label_files:
        ...
        with open(lp, "r", encoding="utf-8") as f:
            for line in f.read().splitlines():
                ...
                if cid is None:
                    # различаем "не найдено" и "неоднозначно"
                    # (если вообще нет ни одного k, содержащего code -> unknown)
        ...
        for (cid, _xc, _yc, _w, _h) in labels:
            index["images_by_class"].setdefault(cid, set()).add(img_path)
            index["instances_per_class"][cid] = index["instances_per_class"].get(cid, 0) + 1
...
def import_external_images_for_selected(out_dir, external_index, selected_ids, id_to_code, train_list, val_list, ann_w, unique_idx_ref):
    """
    Подмешиваем внешние изображения:
    - берём все картинки, где встречается выбранный class_id
    - копируем/конвертируем в наш dataset_<N>/images
    - пишем labels в наш dataset_<N>/labels (с нашими class_id)
    - обновляем train.txt/val.txt и annotations.csv
    Возвращаем:
    - imported_counts: сколько экземпляров добавили по class_id
    - imported_class_ids: какие class_id встретились во внешних изображениях
    """
    imported_counts = {}
    ...
    for img_path in sorted(images_to_import):
        ...
        # конвертим/копируем в PNG и даём имя по классам
...
def compute_target_count(selected_classes, external_instances_per_class=None):
    """
    Минимальное целевое число экземпляров на класс:
    max(MIN_IMAGES_PER_CLASS, max(2*V) среди вариативных классов, max_instances_external).
    max_instances_external считаем по выбранным классам.
    """
    ...
    return int(max(MIN_IMAGES_PER_CLASS, max_variation_target, max_external))
...
def degrade_sign_by_size(sign_rgba, sign_bbox_w_px):
    ...
    t = clamp(t, 0.0, 1.0)
    # Даже для больших знаков оставляем деградацию (качество не бывает идеальным)
    t = clamp(t * float(SIGN_BASE_QUALITY), 0.0, 1.0)
    ...
    # 0) Пикселизация (особенно для маленьких знаков)
    if SIGN_PIXELATE_ENABLED and random.random() < SIGN_PIXELATE_PROB:
        s_pix = sample_interp_range(SIGN_PIXELATE_SCALE_SMALL, SIGN_PIXELATE_SCALE_LARGE, t)
...
def generate_multi(bg_paths, main_state, states, extra_pool_states, template_cache):
    ...
    states: список состояний всех классов (для размещения и подсчёта).
    extra_pool_states: из каких классов можно брать ДОП. объекты.
    ...
    for _ in range(extra_n):
        ...
        if random.random() < EXTRA_SAME_CLASS_PROB:
            ...
            candidates = [s for s in extra_pool_states if s["count"] < s["target"]]
            if not candidates:
...
def generate_dataset():
    ...
    # Загружаем ВСЕ классы (для корректного classes.txt / dataset.yaml и для "смежных" классов из внешнего датасета)
    all_classes = load_classes(filter_ids=None)

    # Какие классы считаем "выбранными для генерации"
    if SELECT_CLASS_IDS:
        selected_ids = sorted(set(int(x) for x in SELECT_CLASS_IDS))
    else:
        selected_ids = [int(c["class_id"]) for c in all_classes]

    selected_classes = [c for c in all_classes if int(c["class_id"]) in set(selected_ids)]
    if not selected_classes:
        raise RuntimeError("Нет выбранных классов для генерации (проверьте SELECT_CLASS_IDS).")

    # Для сопоставления внешнего датасета по имени класса
    ...
    if EXTERNAL_MIX_ENABLED and os.path.isdir(EXTERNAL_DATASET_DIR):
        external_index = load_external_index(EXTERNAL_DATASET_DIR, our_code_to_id)

    # TARGET по экземплярам: учитываем вариации и внешний датасет (по выбранным классам)
    ...
    # classes.txt и dataset.yaml должны знать все class_id, которые могут встретиться (включая смежные из внешнего)
    ...
    # Для выбранных классов target = target_instances, для остальных target = 0 (если нет внешних данных).
    state_by_id = {}
    for c in all_classes:
        cid = int(c["class_id"])
        if cid in set(selected_ids):
            seq = build_template_sequence(c, target_instances)
            state_by_id[cid] = {"cls": c, "seq": seq, "ptr": 0, "count": 0, "target": target_instances}
        else:
            # Не выбранный класс: генерацию не требуем.
            state_by_id[cid] = {"cls": c, "seq": [random.choice(c["templates"])], "ptr": 0, "count": 0, "target": 0}

    states = list(state_by_id.values())

    # Подмешиваем внешний датасет (если есть). Это увеличивает count по всем классам, которые встретились.
    if external_index.get("enabled"):
        unique_idx_ref = [unique_idx]
        imported_counts, imported_ids = import_external_images_for_selected(
            out_dir=out_dir,
        ...
        # обновим unique_idx (после копирования внешних изображений)
        unique_idx = unique_idx_ref[0]
        for cid, cnt in imported_counts.items():
            if cid in state_by_id:
                state_by_id[cid]["count"] += int(cnt)
                # если класс не выбран, но появился во внешнем — не заставляем его добирать синтетикой
    ...
    try:
        ...
        # Считаем целевое число объектов только по выбранным классам
        selected_states = [state_by_id[cid] for cid in set(selected_ids) if cid in state_by_id]
        total_target = len(selected_states) * target_instances
        total_made = 0
        attempts = 0
        max_attempts = total_target * MAX_ATTEMPTS_MULT

        # Из каких классов можно брать доп. объекты на кадре
        if SELECT_CLASS_IDS and not ALLOW_EXTRA_NON_SELECTED_CLASSES:
