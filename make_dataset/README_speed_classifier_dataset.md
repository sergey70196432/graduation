...
- `stats.txt` сохраняет ключевые параметры генерации, включая bad-crops, реальные фоны и статистику по real ROI.
...
### Что сейчас делает генератор помимо базовой синтетики

- использует реальные дорожные фоны из `make_dataset/dashcam_frames` и `make_dataset/backgrounds`;
- генерирует усиленные `bad crop`-примеры, похожие на неточные ROI после детектора;
- может подмешивать реальные ROI в train-сплит для уменьшения domain shift;
- при честной постановке real ROI должны попадать только в train-часть (`roi_index_train.jsonl`), а итоговая оценка должна считаться по отдельному holdout test.
